...
| **P-TP-2** | TP | `resolveEntities` **same-call idempotent** (not fully convergent in general): re-applying the **identical** `(entity_ids, canonical_id)` call leaves the durable alias→canonical mapping unchanged — re-applying the identical call does **not** flip an alias's canonical. *(Scope notes: (a) an overlapping re-resolution with a **DIFFERENT** canonical **legally flips** aliases; (b) `resolve_entities` never **REMOVES** a prior alias entry (`src/store/mod.rs` only `insert`s a durable alias→canonical record, never deletes), so after an overlapping re-resolution a **residual alias** can persist and full convergence to the original canonical is **NOT** guaranteed — **same-call idempotence holds; overlap convergence does not** (verified by a PBT probe).)* | `gen_entities` | After `resolveEntities(ids, {canonical_id: c})`, snapshot `entityAliasCanonical(a)` for every alias `a`; re-apply the identical call, then re-read — every recorded alias still maps to the same canonical, and the two `ResolutionResult`s (merged + aliases) are equal. |
