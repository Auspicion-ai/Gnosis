...
| Unit | Status | Notes |
| --- | --- | --- |
| **The RAG/agent-memory retrieval stack** (spec §4.5–§4.6) | ready to delegate (next) | The query modes (`flat`/`graph`/`vector`/`hybrid`), `ragQuery`/`ragStream`, the retrieval stack (lexical BM25, reranking, multi-query, compression, HyDE, sub-task DAG), multiple vector fields + the vector index/embedding provider, the provenance `trace`, the engine-side query **audit-log recording sink** (the §4.3 `getQueryAuditLog` accessor exists; the recording feed lands here), the agent-memory surface (facts table + **profile summary**), and cross-field consistency (deferred §4.5). Must honor the uniform lock order + IMMUTABLE-DERIVED-SNAPSHOT for the indexes. |
