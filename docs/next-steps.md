...
below) — **360 green tests** (baseline 295 + the F2 wire set 65:
`wire_conformance.rs` 57 [47 conformance + 2 adversarial regressions + 8
adversarial probes] + `props_wire.rs` 8), with `build`/`clippy`/`fmt` clean.
...
| Unit | Status | Notes |
| --- | --- | --- |
| **_(none — no remaining OPEN unit in this repo's current scope)_** | **ALL DONE** | The core engine (§4.1–§4.6) **and** the §7.2/F2 engine wire contract are complete, adversarially hardened, and trio-green (**360 tests**). Two items remain outstanding but are explicitly **deferred / out of this repo's current scope** and recorded where noted: **(1) the shell-integration unit** — server host + HTTP-over-native-IPC transport decision, shell-side SSE client, bind/auth/TLS + loopback, full `RagStore` CRUD routing, engine boot→READY lifecycle + end-to-end transport test, HTTP-status rendering, and D2 engine-absent shell behavior — owned by a later Astrographer-shell unit (recorded in `docs/specs/7-2-f2-review.md` §"Deferred / OUT of F2" + §"What a later shell-integration unit must own", `docs/specs/engine-wire-contract.md` §2/§14, and `docs/decisions.md` `F2-WIRE-CONTRACT-A1`); **(2) the deferred UUID-v4 id decision** — engine-assigned `documentId`/`wikiId` are not RFC-4122 UUID v4; ids cross the wire as **opaque strings** via the `schemaVersion`/`idFormat` seam (`"opaque-string-v1"` is the only implemented format), and RFC-4122 adoption is a suite/HANDOFF reconciliation (recorded in `docs/pending.md` §DOC-REVIEW GAPS row "Engine-assigned `documentId` (and `wikiId`) are not UUID v4", `docs/defects.md`, and `docs/specs/engine-wire-contract.md` §4.1). A fresh supervisor picks up either of these next; neither requires new engine-`src/` work in this repo as scoped. |
