...
| Unit | Status | Notes |
| --- | --- | --- |
| **F6 — RAG evaluation harness** (spec §7.8) | **RE-SCOPED + UNPARKED (2026-09-09)** — proposal-review **PASS** (decision F6-EVAL-RE-SCOPED, `docs/specs/6-f6-eval-review.md`) | A **dev/QA gate**, not a runtime contract element. The as-stated F6 (audit-log-fed, faithfulness/relevancy, DeepEval-in-Rust) is **invalid** (audit log records only counts; engine produces no answer; DeepEval metrics are LLM-as-judge). Re-scoped to a **deterministic, ground-truth-labeled contextual precision/recall module**: `src/retrieval/eval.rs` (`contextual_precision`/`contextual_recall`/`ndcg_at_k`/`mrr_at_k`) + a `gnosis-eval` `[[bin]]` + `tests/eval_harness.rs` + `tests/props_eval.rs` + a labeled-corpus fixture. **Zero runtime change** (no §4.3.4/§4.5/§4.6 surface change; zero new runtime deps); harness re-runs `rag_query` against a READY engine. Faithfulness/relevancy + DeepEval Python harness + answer generation are **suite-side**. Awaiting go-ahead → spec-writer → red → implementer → adversarial → trio. |
