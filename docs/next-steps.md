...
**Handover state (current): the core engine, the wire-contract seam, AND the F6
RAG-evaluation dev/QA gate are fully implemented, adversarially hardened, and
trio-green.** §4.1–§4.6 (the core engine), the §7.2/F2 engine wire contract, and
the §7.8/F6 RAG-evaluation harness are all DONE (see the DONE rows below) —
**387 green tests** (baseline 368 + the F6 eval set 19: `eval_harness.rs` 11
[9 red-set + 1 already-green `query_mode_serde_pascal_case` + 1 adversarial-fix
bin smoke `bin_smoke_report_shape`] + `props_eval.rs` 8), with
`build`/`clippy`/`fmt` clean.
**The remaining unparked item in this repo's scope is F4 — scoped community
summaries** (spec §7.5): the **manual** `declareCommunity` override is
implemented (pinned); the scoped automatic summary surface is the partial-unpark
path (the full hierarchical GraphRAG pipeline stays PARKED for D2 cost/latency).
A fresh supervisor picks up F4 (proposal-review → spec → TDD) next. The deferred
**shell-integration unit** (server host + SSE client + bind/auth/TLS + full
`RagStore` CRUD + boot→READY lifecycle + HTTP-status rendering + D2-absent shell
behavior), the **deferred UUID-v4 id decision** (ids cross the wire as opaque
strings via the `schemaVersion`/`idFormat` seam), and the **suite-side DeepEval
Python harness** (faithfulness/answer-relevancy, answer generation, user-facing
retrieval-quality surface) remain outstanding and are recorded in
`docs/specs/7-2-f2-review.md` (§"Deferred / OUT of F2" + §"What a later
shell-integration unit must own"), `docs/specs/6-f6-eval-review.md`,
`docs/pending.md`, and `docs/decisions.md` (`F2-WIRE-CONTRACT-A1`,
`F6-EVAL-RE-SCOPED`). The dated entries below are the historical
scaffold→implemented record.
...
| Unit | Status | Notes |
| --- | --- | --- |
| **F4 — Community summaries (retrieval)** (spec §7.5) | **RE-SCOPED TO RETRIEVAL + UNPARKED (2026-09-09)** — proposal-review **PASS** (decision F4-COMMUNITY-RETRIEVAL, `docs/specs/f4-community-summaries-review.md`) | The **derivation** surface is **invalid** (vacuous: `declareCommunity` requires a non-empty manual summary, so every community already has one; redundant: a concatenation duplicates `rag_query` graph mode). Re-scoped to a **retrieval surface**: `getCommunityContext(communityId) → CommunityContext` — a new read-only `RagStore` accessor returning the authoritative manual summary + member set + current `CommunityState`. Fail-state `CommunityNotFound` only. **Zero runtime change** (no new deps, no provider, side-effect-free); §4.2.8.4 manual-override + §4.4.3 staleness preserved; FS-24 stays RESERVED. The **manual** `declareCommunity` override remains pinned. LLM summaries / hierarchical detection / global retrieval / full GraphRAG pipeline stay **suite-side / parked**. Awaiting go-ahead → spec-writer → red → implementer → adversarial → trio. |
| **_(shell-integration unit + UUID-v4 id decision + suite-side DeepEval harness)_** | **DEFERRED / out of this repo's current scope** | The shell-integration unit (server host + HTTP-over-native-IPC transport, SSE client, bind/auth/TLS, full `RagStore` CRUD routing, boot→READY lifecycle, HTTP-status rendering, D2-absent shell behavior) is owned by a later Astrographer-shell unit (recorded in `docs/specs/7-2-f2-review.md` + `docs/specs/engine-wire-contract.md` §2/§14 + `docs/decisions.md` `F2-WIRE-CONTRACT-A1`). The **UUID-v4 id decision** is **RESOLVED** as `ID-SCHEME-RECONCILE-MONOTONIC` (ids stay monotonic opaque strings; RFC-4122 not taken up; reconcile request in `docs/HANDOFF.md`). The **suite-side DeepEval Python harness** (faithfulness/answer-relevancy, answer generation, user-facing retrieval-quality surface) is **NOT Gnosis** — recorded in `docs/specs/6-f6-eval-review.md` + `docs/decisions.md` `F6-EVAL-RE-SCOPED`. |
