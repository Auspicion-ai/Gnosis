...
RAG-evaluation dev/QA gate, AND the F4 community-retrieval unit are fully
implemented, adversarially hardened, and trio-green.** §4.1–§4.6 (the core
engine), the §7.2/F2 engine wire contract, the §7.8/F6 RAG-evaluation harness,
and the §7.5/F4 community-retrieval accessor are all DONE (see the DONE rows
below) — **411 green tests** (baseline 387 + the F4 set 24:
`community_context_integration.rs` 15 [13 happy/fail H-1..H-11/F-1/F-2 + 2
negative probes `neg4`/`neg5`] + `props_community_context.rs` 9 [6 PBT rows
P-IM-1/P-IM-2/P-SM-1/P-SM-2/P-TP-1/P-TP-2 + 3 negative probes `neg1`/`neg2`/
`neg3`]), with `build`/`clippy`/`fmt` clean.
...
`ID-SCHEME-RECONCILE-MONOTONIC`), F6, and F4. A fresh supervisor picks up the
...
behavior) next. The **suite-side DeepEval Python harness**
